...
import {renderElementName} from 'client/utils/elements';
...
const SlideshowListPage: React.FC<IScreensPageData> = (props) => {
	const {loading, slideShow, elementId, element, permissions} = props;
	...
	useEffect(() => {
		if (elementId) {
			Meteor.call(
				methodNames.statistics.getSlideShowsForElement,
				elementId,
							...
							{[slideShow].map((slideshow, index) => (
								<React.Fragment key={slideshow._id}>
									<TableRow key={slideshow._id}>
										...
										<TableCell>
											{groupsStatisticsTotal[index]?.daysPassed
												? `${
														groupsStatisticsTotal[index]?.daysPassed
												  } ${pluralize(
														groupsStatisticsTotal[index]?.daysPassed,
												...
												: groupsStatisticsTotal[index]?.daysPassed === 0
												? 'Сегодня'
												: ''}
										</TableCell>
										{/*<TableCell>*/}
	...
	return (
		<div className={css.container}>
			<PageTitle
				title={renderElementName(element?.type)}
...
export default withTracker<IScreensPageData, RouteComponentProps>(({match}) => {
	...
	const subMyList = Meteor.subscribe(publishNames.slideshow.byElement, id).ready();
	...
	const element = SlideElement.findOne({_id: id});
